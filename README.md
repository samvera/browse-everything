...
<tbody>
<tr valign="top">
<td><strong>JavaScript API</strong></td>
<td>
...
<button id="browse-btn"
        ...
        data-target="#your-form">
  Browse
</button>
```
</td>
<td>The button will automatically initialize <code>browse-everything</code> when the page loads. If programmatic callbacks are needed for a more customized implementation, they can be added in the JavaScript after <code>browse-everything</code> has been initialized. Example in the <code>dummy_test_app</code> <a href="https://github.com/samvera/browse-everything/blob/main/spec/dummy_test_app/app/views/file_handler/main.html.erbp">implementation</a> in <code>browse-everything</code> GitHub repo.</td>
</tr>
<tr valign="top">
<td><strong>CSS Classes</strong></td>
<td><code>.treetable-*</code> classes</td>
<td><code>.wb-*</code> classes from new JS library</td>
<td>Update any custom styles to use new class names that are breaking.</td>
</tr>
<tr valign="top">
...
<td>Update `package.json by adding the fonts path to the Sass build scripts:
