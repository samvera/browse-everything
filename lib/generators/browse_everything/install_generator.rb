...
class BrowseEverything::InstallGenerator < Rails::Generators::Base
  ...
  def install_webpack_config
    copy_file 'config/webpack/development.js', 'config/webpack/development.js'
  end
  ...
  def install_swagger_tests
    FileUtils.mkdir_p Rails.root.join('swagger', 'spec', 'integration')
    pattern = Rails.root.join('..', 'swagger', 'spec', 'integration', '*_spec.rb').to_s
  ...
  def install_javascript_packages
    ...
    system('yarn install')
    system('ln -s /Usr/jrgriffiniii/src/my_browse_everything-react node_modules/browse_everything')
  end
