    ...
    class Github < Base
      ...
      def validate_config
        unless config[:access_token]
          raise BrowseEverything::InitializationError, 'Octokit driver requires a :access_token argument'
        end
        unless config[:repository]
          raise BrowseEverything::InitializationError, 'Octokit driver requires a :repository argument'
        end
      end
